// This is a modified png2theora program that comes with libtheora.
// It adds -Z flag that makes the program to output binary chunks
// that make up the video (useful for testing streaming) rather than
// the video itself.
// The chunks get written to fragment-%03d.dat files.

// Compile like this:
// gcc png2theora.c -o png2theora -ltheoraenc -ltheoradec -lpng
...
#define PROGRAM_VERSION  "1.1-stackvm"

static const char *option_output = NULL;
static int video_fps_numerator = 0;
static int video_fps_denominator = 0;
...
static int write_fragments = 0;

void write_stream_fragment(void *data, int len) {
    static int stream_fragment = 0;
    char filename[1024];
    snprintf(filename, 1024, "fragment-%03d.dat", stream_fragment++);
    FILE *out = fopen(filename, "w+");
    if (!out) {
        fprintf(stderr, "write_stream_fragment failed at %d\n", stream_fragment);
        exit(1);
    }
...
const char *optstring = "o:hv:\4:\2:V:s:S:f:F:ck:d:\1\2\3\4\5\6Z";
struct option options [] = {
 ...
 {"stream-fragments",no_argument,NULL,'Z'},
 {NULL,0,NULL,0}
};
...
static void usage(void){
  fprintf(stderr,
          ...
          "  -Z --stream-fragments           Write out all the fragments the library\n"
          "                                  generates for video\n"
...
theora_write_frame(unsigned long w, unsigned long h, unsigned char *yuv, int last)
{
  ...
  if (passno!=1) { 
    ...
    while(ogg_stream_pageout(&ogg_os, &og)) {
      ...
      if (write_fragments) write_stream_fragment(og.header, og.header_len);
      fwrite(og.body, og.body_len, 1, ogg_fp);
      if (write_fragments) write_stream_fragment(og.body, og.body_len);
    }
  }  
